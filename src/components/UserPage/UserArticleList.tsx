...
import articleListReducer, { fetchArticleListUser } from '../../reducer/articleListReducer'
...
const UserArticleList = () => {
  ...
  useEffect(() => {
    dispatch(fetchArticleListUser({ userId, next: 0 }))
    return () => {
  ...
  const fetchData = () => {
    try {
      if (articleList.hasMore) {
        setNext(next + 12)
        dispatch(fetchArticleListUser({ userId, next }))
      }
    } catch (error) {
      console.log(error)
    }
